...
%% Kinematik
d = 95*2*1e-3; %Angriffspunkt des Gimbal-Motors
x0 = 94.5*1e-3; %Abstand des oberen Gabelpunktes zum Globaeln Koordinatensys.
l = 40*1e-3; %Länge der Gabel
a = 27*1e-3; %Höhe unteres gabelauge
...
sprintf(join(stringM), max([max(MA1),max(MA2)])*1e2 )
sprintf(join(stringF), max([max(Fay1),max(Fay2)]))
sprintf(join(stringP), max([max(Pow1),max(Pow2)]))
sprintf(join(stringI), max([max(I1),max(I2)]))
